...
export default function UserHeader({ user: propUser, isMe }) {
  ...
  useEffect(() => {
    const loadRelations = async () => {
      ...
      console.log('🔄 Loading friend relations for targetId:', targetId);
      ...
      try {
        ...
        // Friend status - fetch all friend relations with no cache
        ...
        console.log('📋 Friends list:', getListData(friendsRes.data));
        console.log('📬 Incoming requests:', getListData(incomingRes.data));
        console.log('📤 Outgoing requests:', getListData(outgoingRes.data));

        // Check friends list first
        const friends = getListData(friendsRes.data) || [];
        const isFriend = friends.some((f) => {
          const friendIds = getRelationIds(f);
          console.log('  Checking friend:', f, 'friendIds:', friendIds, 'targetId:', targetId);
          return friendIds.some((id) => sameId(id, targetId));
        });

        if (isFriend) {
          console.log('✅ Friend status detected');
        ...
        const incomingReq = incoming.find((r) =>
          getRelationIds(r).some((id) => sameId(id, targetId))
        );
        if (incomingReq) {
          console.log('📬 Incoming request detected:', incomingReq);
          setFriendStatus('incoming');
          setFriendRequestId(
            incomingReq.id || incomingReq.requestId || incomingReq.requestID || null
          );
          return;
        }
        ...
        if (outgoingReq) {
          console.log('📤 Outgoing request detected:', outgoingReq);
  ...
  // Force immediate refetch when target user changes (e.g., navigating profiles)
  useEffect(() => {
    const targetId = (propUser || fetchedUser)?.id || (propUser || fetchedUser)?.userId;
    const targetName = (propUser || fetchedUser)?.fullName || (propUser || fetchedUser)?.name;

    if (!isMe && targetId && targetName) {
      console.log(
        '🚀 Target user changed, force refetching friend status for:',
        targetName,
        `(${targetId})`
      );

      // Trigger refetch by simulating friendStatusChanged event
      setTimeout(() => {
        const event = new Event('friendStatusChanged');
        window.dispatchEvent(event);
      }, 100);
    }
  }, [propUser?.id || propUser?.userId, fetchedUser?.id || fetchedUser?.userId, isMe]);

  // Listen for friend status changes from other components
  useEffect(() => {
    const handleFriendStatusChange = () => {
      const targetId = (propUser || fetchedUser)?.id || (propUser || fetchedUser)?.userId;
      if (!isMe && targetId) {
        // Refetch friend relations after a small delay
        setTimeout(() => {
          const refetch = async () => {
            try {
              const [friendsRes, incomingRes, outgoingRes] = await Promise.all([
              ...
              const friends = getListData(friendsRes.data) || [];
              const isFriend = friends.some((f) =>
                getRelationIds(f).some((id) => sameId(id, targetId))
              );
              if (isFriend) {
                setFriendStatus('friend');
                setFriendRequestId(null);
              ...
              console.error('Failed to refetch friend status:', err);
            }
          };
          refetch();
        }, 200);
    ...
    // Auto-refresh friend status every 10 seconds when viewing other profile (was 30s)
    let pollInterval;
    if (!isMe && (propUser || fetchedUser)) {
      console.log('⏰ Setting up polling interval for friend status');
      pollInterval = setInterval(() => {
        console.log('⏰ Polling: refetching friend status for targetId');
  ...
  const handleMessage = async () => {
    ...
    try {
      ...
      // Use conversationId (camelCase) from BE response
      const conversationId = conversation?.conversationId;
      if (!conversationId) {
        console.error('No conversation ID found in response:', conversation);
        message.error('Could not create conversation');
        return;
      }

      navigate(`/customer/message/${String(conversationId)}`, {
