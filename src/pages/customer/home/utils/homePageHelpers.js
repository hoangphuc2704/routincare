...
export function normalizePostItem(item, index, options = {}) {
  ...
  const createdAtRaw = item?.createdAt || item?.updatedAt || item?.publishedAt || null;
  const parsedCreatedAt = createdAtRaw ? new Date(createdAtRaw).getTime() : NaN;
  const sortTimestamp = Number.isFinite(parsedCreatedAt) ? parsedCreatedAt : 0;
  ...
  return {
    ...
    sortTimestamp,
    timeago: toRelativeTime(createdAtRaw),
    user: {
      id: normalizedUserId,
      name: userName,
      avatar: userAvatar,
