...
export default function Homepage() {
  ...
  const handleOpenCopyPreview = useCallback(async (target) => {
    ...
    try {
      ...
      message.error(err?.response?.data?.message || 'Không thể tải chi tiết thói quen lúc này.');
    } finally {
  ...
  const handleCopyRoutine = useCallback(async () => {
    ...
    try {
      ...
      message.success('Sao chép thói quen thành công.');
      setPreviewOpen(false);
      setSelectedCopyTarget(null);
      setPreviewRoutine(null);
    } catch (err) {
      message.error(err?.response?.data?.message || 'Không thể sao chép thói quen lúc này.');
    } finally {
        ...
        {loading ? (
          ...
          feedPosts.length > 0 ? (
            <div className="flex flex-col gap-3">
              ...
              {!feedHasMore && (
                <p className="text-center text-xs text-white/45 py-2">Đã hiển thị hết bảng tin.</p>
              )}
            </div>
          ) : (
            <div className="px-4 py-10 text-center text-white/60 rounded-3xl border border-white/10 bg-black/20">
              <p className="text-lg font-semibold">Bảng tin của bạn đang trống</p>
          ...
          <div className="space-y-6">
            <section>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-lg font-bold">Thói quen nổi bật</h2>
              </div>
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <div className="mt-3 flex items-center justify-between gap-3">
                        ...
                        >
                          {copyLoadingByPost[`explore-${routine.id}`] ? 'Đang sao chép...' : 'Sao chép thói quen'}
                        </button>
                      </div>
                    </div>
                  </article>
                ...
                {exploreUsers.map((user, index) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-[#d2fb05] mt-1">{user.followers} người theo dõi</p>
                    </div>
                  </Link>
                ))}
              </div>
